...
def get_columns():
	return [
		...
		{
			"fieldname": "shifaa_service_unit",
			"fieldtype": "Link",
			"label": "Shifaa Service Unit",
		...
		{
			"fieldname": "shifaa_practitioner",
			"fieldtype": "Link",
			"label": "Shifaa Practitioner",
...
def get_inpatient_details(data, service_unit):
	...
	for entry in data:
		entry["shifaa_service_unit"] = get_current_shifaa_service_unit(entry.inpatient_record)
		if entry.is_completed:
			entry["inpatient_medication_entry"] = get_inpatient_medication_entry(entry.name)

		if (
			service_unit and entry.shifaa_service_unit and service_unit != entry.shifaa_service_unit
		):
