	...
	refresh: function(frm) {
		...
		frm.set_query('dt', 'items', function() {
			if (['Department', 'Practitioner'].includes(frm.doc.allow_booking_for)) {
				...
				return {
					filters: {'name': ['=', 'Shifaa Service Unit']}
				};
			}
		});
		...
		frm.set_query('dn', 'items', function(doc, cdt, cdn) {
			...
			} else if (child.dt === 'Shifaa Service Unit') {
				let item_list = doc.items
					.filter(item => item.dt === 'Shifaa Service Unit')
					.map(({dn}) => dn);
				return {
					filters: [
						['Shifaa Service Unit', 'name', 'not in', item_list],
						['Shifaa Service Unit', 'allow_appointments', "=", 1],
					]
				};
			}
		});
