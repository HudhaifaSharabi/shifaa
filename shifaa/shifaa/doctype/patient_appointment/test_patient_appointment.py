...
from shifaa.shifaa.doctype.patient_appointment.patient_appointment import (
...
class TestPatientAppointment(FrappeTestCase):
	def setUp(self):
		...
		make_pos_profile()
		frappe.db.sql("""delete from `tabShifaa Service Unit` where name like '_Test %'""")
		frappe.db.sql(
			"""delete from `tabShifaa Service Unit` where name like '_Test Service Unit Type%'"""
		)

	def test_status(self):
		patient, practitioner = create_shifaa_docs()
		frappe.db.set_single_value("Shifaa Settings", "show_payment_popup", 0)
	...
	def test_start_encounter(self):
		patient, practitioner = create_shifaa_docs()
		frappe.db.set_single_value("Shifaa Settings", "show_payment_popup", 1)
	...
	def test_auto_invoicing(self):
		patient, practitioner = create_shifaa_docs()
		frappe.db.set_single_value("Shifaa Settings", "enable_free_follow_ups", 0)
		frappe.db.set_single_value("Shifaa Settings", "show_payment_popup", 0)
		appointment = create_appointment(patient, practitioner, nowdate())
	...
	def test_auto_invoicing_based_on_department(self):
		frappe.db.set_single_value("Shifaa Settings", "enable_free_follow_ups", 1)
		frappe.db.set_single_value("Shifaa Settings", "show_payment_popup", 1)
		item = create_shifaa_service_items()
		...
		if frappe.db.get_single_value("Shifaa Settings", "show_payment_popup"):
	...
	def test_appointment_booking_for_admission_service_unit(self):
		from shifaa.shifaa.doctype.inpatient_record.inpatient_record import (
			admit_patient,
			discharge_patient,
			schedule_discharge,
		)
		from shifaa.shifaa.doctype.inpatient_record.test_inpatient_record import (
			create_inpatient,
			get_shifaa_service_unit,
		...
		# Admit
		service_unit = get_shifaa_service_unit("_Test Service Unit Ip Occupancy")
	...
	def test_invalid_shifaa_service_unit_validation(self):
		from shifaa.shifaa.doctype.inpatient_record.inpatient_record import (
			admit_patient,
			discharge_patient,
			schedule_discharge,
		...
		appointment_service_unit = get_shifaa_service_unit(
	...
	def test_payment_should_be_mandatory_for_new_patient_appointment(self):
		frappe.db.set_single_value("Shifaa Settings", "enable_free_follow_ups", 1)
		frappe.db.set_single_value("Shifaa Settings", "show_payment_popup", 1)
		frappe.db.set_single_value("Shifaa Settings", "max_visits", 3)
		frappe.db.set_single_value("Shifaa Settings", "valid_days", 30)
	...
	def test_patient_appointment_should_consider_permissions_while_fetching_appointments(self):
		...
		patient, new_practitioner = create_shifaa_docs(id=5)
		create_appointment(patient, new_practitioner, nowdate())

		roles = [{"doctype": "Has Role", "role": "Physician"}]
		user = create_user(roles=roles)
		new_practitioner = frappe.get_doc("Shifaa Practitioner", new_practitioner)
	...
	def test_overlap_appointment(self):
		from shifaa.shifaa.doctype.patient_appointment.patient_appointment import OverlapError

		patient, practitioner = create_shifaa_docs(id=1)
		patient_1, practitioner_1 = create_shifaa_docs(id=2)
...
def create_practitioner(id=0, medical_department=None):
	if frappe.db.exists(
		"Shifaa Practitioner", {"firstname": f"_Test Shifaa Practitioner {str(id)}"}
	):
		practitioner = frappe.db.get_value(
			"Shifaa Practitioner", {"firstname": f"_Test Shifaa Practitioner {str(id)}"}, ["name"]
		)
		return practitioner

	practitioner = frappe.new_doc("Shifaa Practitioner")
	practitioner.first_name = f"_Test Shifaa Practitioner {str(id)}"
...
def create_appointment(
	...
	item = create_shifaa_service_items()
	frappe.db.set_single_value("Shifaa Settings", "inpatient_visit_charge_item", item)
	frappe.db.set_single_value("Shifaa Settings", "op_consulting_charge_item", item)
	appointment = frappe.new_doc("Patient Appointment")
	...
	if save:
		appointment.save(ignore_permissions=True)
		if invoice or frappe.db.get_single_value("Shifaa Settings", "show_payment_popup"):
			invoice_appointment(appointment.name, discount_percentage, discount_amount)

	return appointment


def create_shifaa_service_items():
...
def create_service_unit_type(id=0, allow_appointments=1, overlap_appointments=0):
	if frappe.db.exists("Shifaa Service Unit Type", f"_Test Service Unit Type {str(id)}"):
		return f"_Test Service Unit Type {str(id)}"

	service_unit_type = frappe.new_doc("Shifaa Service Unit Type")
...
def create_service_unit(id=0, service_unit_type=None, service_unit_capacity=0):
	if frappe.db.exists("Shifaa Service Unit", f"_Test Service Unit {str(id)}"):
		return f"_Test service_unit {str(id)}"

	service_unit = frappe.new_doc("Shifaa Service Unit")
	service_unit.is_group = 0
	service_unit.shifaa_service_unit_name = f"_Test Service Unit {str(id)}"
