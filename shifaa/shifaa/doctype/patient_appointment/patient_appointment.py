...
from shifaa.shifaa.doctype.fee_validity.fee_validity import (
	check_fee_validity,
	get_fee_validity,
	manage_fee_validity,
)
...
from shifaa.shifaa.utils import get_appointment_billing_item_and_rate
...
class PatientAppointment(Document):
	...
	def validate_overlaps(self):
		...
		if self.service_unit:  # validate service unit capacity if overlap enabled
			allow_overlap, service_unit_capacity = frappe.get_value(
				"Shifaa Service Unit", self.service_unit, ["overlap_appointments", "service_unit_capacity"]
			)
			if allow_overlap:
	...
	def validate_service_unit(self):
		if self.inpatient_record and self.service_unit:
			from shifaa.shifaa.doctype.inpatient_medication_entry.inpatient_medication_entry import (
				get_current_shifaa_service_unit,
			)

			is_inpatient_occupancy_unit = frappe.db.get_value(
				"Shifaa Service Unit", self.service_unit, "inpatient_occupancy"
			)
			service_unit = get_current_shifaa_service_unit(self.inpatient_record)
			if is_inpatient_occupancy_unit and service_unit != self.service_unit:
				...
				frappe.throw(msg, title=_("Invalid Shifaa Service Unit"))
	...
	def insert_calendar_event(self):
		...
		google_calendar = frappe.db.get_value(
			"Shifaa Practitioner", self.practitioner, "google_calendar"
		)
		if not google_calendar:
			google_calendar = frappe.db.get_single_value("Shifaa Settings", "default_google_calendar")
		...
		participants.append(
			{"reference_doctype": "Shifaa Practitioner", "reference_docname": self.practitioner}
		)
...
def check_payment_reqd(patient):
	...
	"""
	show_payment_popup = frappe.db.get_single_value("Shifaa Settings", "show_payment_popup")
	free_follow_ups = frappe.db.get_single_value("Shifaa Settings", "enable_free_follow_ups")
...
def invoice_appointment(appointment_name, discount_percentage=0, discount_amount=0):
	...
	settings = frappe.get_single("Shifaa Settings")
...
def update_fee_validity(appointment):
	...
	if (
		not frappe.db.get_single_value("Shifaa Settings", "enable_free_follow_ups")
...
def cancel_appointment(appointment_id):
	...
	if appointment.invoiced:
		...
		if frappe.db.get_single_value("Shifaa Settings", "enable_free_follow_ups"):
...
def get_availability_data(date, practitioner, appointment):
	...
	practitioner_doc = frappe.get_doc("Shifaa Practitioner", practitioner)
	...
	if practitioner_doc.practitioner_schedules:
		...
		frappe.throw(
			_(
				"{0} does not have a Shifaa Practitioner Schedule. Add it in Shifaa Practitioner master"
			).format(practitioner),
	...
	if not slot_details:
		...
		frappe.throw(
			_("Shifaa Practitioner not available on {0}").format(weekday), title=_("Not Available")
		)
		...
		if not schedule_entry.service_unit:
			frappe.throw(
				_(
					...
					get_link_to_form("Shifaa Practitioner", practitioner),
...
def send_confirmation_msg(doc):
	if frappe.db.get_single_value("Shifaa Settings", "send_appointment_confirmation"):
		message = frappe.db.get_single_value("Shifaa Settings", "appointment_confirmation_msg")
		try:
...
def send_appointment_reminder():
	if frappe.db.get_single_value("Shifaa Settings", "send_appointment_reminder"):
		remind_before = datetime.datetime.strptime(
			frappe.db.get_single_value("Shifaa Settings", "remind_before"), "%H:%M:%S"
		)
		...
		for appointment in appointment_list:
			...
			message = frappe.db.get_single_value("Shifaa Settings", "appointment_reminder_msg")
			send_message(doc, message)
