...
from shifaa.shifaa.doctype.shifaa_settings.shifaa_settings import get_account
from shifaa.shifaa.doctype.lab_test.lab_test import create_sample_doc
from shifaa.shifaa.doctype.nursing_task.nursing_task import NursingTask
from shifaa.shifaa.doctype.service_request.service_request import (
	update_service_request_status,
)
from shifaa.shifaa.utils import validate_nursing_tasks
...
class ClinicalProcedure(Document):
	...
	def make_material_receipt(self, submit=False):
		...
		expense_account = get_account(None, "expense_account", "Shifaa Settings", self.company)
		for item in self.items:
...
def make_stock_entry(doc):
	...
	expense_account = get_account(None, "expense_account", "Shifaa Settings", doc.company)
	...
	def set_missing_values(source, target):
		...
		if consume_stock:
			...
			if source.service_unit:
				warehouse = frappe.db.get_value("Shifaa Service Unit", source.service_unit, "warehouse")
			if not warehouse:
