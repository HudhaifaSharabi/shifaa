...
class ShifaaServiceUnitType(Document):
	def validate(self):
		if self.allow_appointments and self.inpatient_occupancy:
			frappe.msgprint(
				_("Shifaa Service Unit Type cannot have both {0} and {1}").format(
			...
			frappe.msgprint(
				_("Shifaa Service Unit Type must allow atleast one among {0} and {1}").format(
					frappe.bold("Allow Appointments"), frappe.bold("Inpatient Occupancy")
				),
				raise_exception=1,
				title=_("Validation Error"),
...
def change_item_code(item, item_code, doc_name):
	if frappe.db.exists({"doctype": "Item", "item_code": item_code}):
		...
		frappe.db.set_value("Shifaa Service Unit Type", doc_name, "item_code", item_code)
