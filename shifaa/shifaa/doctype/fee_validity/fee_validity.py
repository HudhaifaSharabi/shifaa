...
def create_fee_validity(appointment):
	...
	fee_validity.max_visits = frappe.db.get_single_value("Shifaa Settings", "max_visits") or 1
	valid_days = frappe.db.get_single_value("Shifaa Settings", "valid_days") or 1
...
def check_fee_validity(appointment, date=None, practitioner=None):
	if not frappe.db.get_single_value("Shifaa Settings", "enable_free_follow_ups"):
		return
			...
			if getdate(appointment.appointment_date) != start_date:
				frappe.db.set_value(
					...
					invoiced_fee_validity,
					{
						...
						+ datetime.timedelta(
							days=int(frappe.db.get_single_value("Shifaa Settings", "valid_days") or 1)
						),
					},
				)
