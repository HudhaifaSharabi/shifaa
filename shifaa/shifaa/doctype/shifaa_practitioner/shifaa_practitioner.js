...
frappe.ui.form.on('Shifaa Practitioner', {
	setup: function(frm) {
		...
		frm.set_query('practitioner_primary_contact', function(doc) {
			return {
				filters: {
					'link_doctype': 'Shifaa Practitioner',
					'link_name': doc.name
				}
			}
		})
	...
	supplier: function (frm) {
		if (frm.doc.supplier) {
			frappe.call({
				method: 'shifaa.shifaa.doctype.shifaa_practitioner.shifaa_practitioner.get_supplier_and_user',
				args: {
	...
	refresh: function(frm) {
		frappe.dynamic_link = {doc: frm.doc, fieldname: 'name', doctype: 'Shifaa Practitioner'};
...
frappe.tour['Shifaa Practitioner'] = [
	{
		fieldname: 'employee',
		title: __('Employee'),
	...
	},
	{
		...
		description: __('If this Shifaa Practitioner works for the In-Patient Department, create a service item for Inpatient Visits.')
	},
	{
	...
	},
	{
		...
		description: __('If this Shifaa Practitioner also works for the In-Patient Department, set the inpatient visit charge for this Practitioner.')
	}
];
