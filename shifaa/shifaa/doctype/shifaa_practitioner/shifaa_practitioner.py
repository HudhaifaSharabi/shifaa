...
class ShifaaPractitioner(Document):
	def onload(self):
	...
	def autoname(self):
		...
		if frappe.db.exists("Shifaa Practitioner", self.name):
	...
	def validate(self):
		...
		if self.user_id:
			...
			existing_user_id = frappe.db.get_value("Shifaa Practitioner", self.name, "user_id")
			if existing_user_id:
				frappe.permissions.remove_user_permission(
					"Shifaa Practitioner", self.name, existing_user_id
				)
	...
	def on_update(self):
		if self.user_id:
			frappe.permissions.add_user_permission("Shifaa Practitioner", self.name, self.user_id)
		...
		for practitioner_schedule in self.practitioner_schedules:
			if frappe.db.get_value(
				...
				if not self.google_calendar and not frappe.db.get_single_value(
					"Shifaa Settings", "default_google_calendar"
				):
					frappe.throw(
						_(
							...
							get_link_to_form("Shifaa Settings", "Shifaa Settings", "Shifaa Settings"),
						),
	...
	def validate_user_id(self):
		...
		practitioner = frappe.db.exists(
			"Shifaa Practitioner", {"user_id": self.user_id, "name": ("!=", self.name)}
		)
		if practitioner:
			frappe.throw(
				_("User {0} is already assigned to Shifaa Practitioner {1}").format(
					self.user_id, practitioner
				)
			)

	def on_trash(self):
		delete_contact_and_address("Shifaa Practitioner", self.name)
