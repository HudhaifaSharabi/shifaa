...
from shifaa.shifaa.doctype.lab_test.test_lab_test import create_patient_encounter
from shifaa.shifaa.utils import get_encounters_to_invoice
...
class TestInpatientRecord(FrappeTestCase):
	def test_admit_and_discharge(self):
		...
		# Admit
		service_unit = get_shifaa_service_unit()
		admit_patient(ip_record, service_unit, now_datetime())
		self.assertEqual("Admitted", frappe.db.get_value("Patient", patient, "inpatient_status"))
		self.assertEqual(
			"Occupied", frappe.db.get_value("Shifaa Service Unit", service_unit, "occupancy_status")
		)

		# Discharge
		schedule_discharge(frappe.as_json({"patient": patient}))
		self.assertEqual(
			"Vacant", frappe.db.get_value("Shifaa Service Unit", service_unit, "occupancy_status")
		)
...
def get_shifaa_service_unit(unit_name=None):
	if not unit_name:
		service_unit = get_random(
			"Shifaa Service Unit", filters={"inpatient_occupancy": 1, "company": "_Test Company"}
		)
	else:
		service_unit = frappe.db.exists(
			"Shifaa Service Unit", {"shifaa_service_unit_name": unit_name}
		)

	if not service_unit:
		service_unit = frappe.new_doc("Shifaa Service Unit")
		service_unit.shifaa_service_unit_name = unit_name or "_Test Service Unit Ip Occupancy"
		...
		service_unit_parent_name = frappe.db.exists(
			{
				...
				"shifaa_service_unit_name": "_Test All Shifaa Service Units",
				"is_group": 1,
			}
		)
		if not service_unit_parent_name:
			parent_service_unit = frappe.new_doc("Shifaa Service Unit")
			parent_service_unit.shifaa_service_unit_name = "_Test All Shifaa Service Units"
			parent_service_unit.is_group = 1
			parent_service_unit.save(ignore_permissions=True)
			service_unit.parent_shifaa_service_unit = parent_service_unit.name
		else:
			service_unit.parent_shifaa_service_unit = service_unit_parent_name
...
def get_service_unit_type():
	service_unit_type = get_random("Shifaa Service Unit Type", filters={"inpatient_occupancy": 1})
