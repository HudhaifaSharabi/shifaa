	...
	if inpatient_record.inpatient_occupancies:
		for inpatient_occupancy in inpatient_record.inpatient_occupancies:
			if inpatient_occupancy.left != 1:
				...
				frappe.db.set_value(
					"Shifaa Service Unit", inpatient_occupancy.service_unit, "occupancy_status", "Vacant"
				)
...
def validate_inpatient_invoicing(inpatient_record):
	if frappe.db.get_single_value("Shifaa Settings", "allow_discharge_despite_unbilled_services"):
		return
	...
	if pending_invoices:
		...
		""".format(
			_("Shifaa Service"), _("Documents"), formatted_doc_rows
		)
...
def transfer_patient(inpatient_record, service_unit, check_in):
	...
	frappe.db.set_value("Shifaa Service Unit", service_unit, "occupancy_status", "Occupied")
...
def is_service_unit_billable(service_unit):
	service_unit_doc = frappe.qb.DocType("Shifaa Service Unit")
	service_unit_type = frappe.qb.DocType("Shifaa Service Unit Type")
	result = (
