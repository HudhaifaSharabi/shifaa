...
frappe.ui.form.on('Patient Encounter', {
	...
	refresh: function(frm) {
			...
			if (frm.doc.drug_prescription && frm.doc.inpatient_record && frm.doc.inpatient_status === "Admitted") {
				frm.add_custom_button(__('Inpatient Medication Order'), function() {
					frappe.model.open_mapped_doc({
						method: 'shifaa.shifaa.doctype.patient_encounter.patient_encounter.make_ip_medication_order',
						frm: frm
					});
				},__('Create'));
			}
		...
		if (frappe.meta.get_docfield('Drug Prescription', 'medication').in_list_view === 1) {
			frm.set_query('drug_code', 'drug_prescription', function(doc, cdt, cdn) {
				...
				return {
					query: 'shifaa.shifaa.doctype.patient_encounter.patient_encounter.get_medications_query',
					filters: filters
				};
			});
		}
	...
	set_patient_info: async function(frm) {
		if (frm.doc.patient) {
			let me = frm
			frappe.call({
				method: 'shifaa.shifaa.doctype.patient.patient.get_patient_detail',
				args: {
					patient: frm.doc.patient
				},
				callback: function(data) {
...
var schedule_inpatient = function(frm) {
	...
	var dialog = new frappe.ui.Dialog({
		...
		fields: [
			...
			{fieldtype: 'Link', label: 'Shifaa Practitioner (Primary)', fieldname: 'primary_practitioner', options: 'Shifaa Practitioner', reqd: 1},
			{fieldtype: 'Link', label: 'Shifaa Practitioner (Secondary)', fieldname: 'secondary_practitioner', options: 'Shifaa Practitioner'},
			...
			{fieldtype: 'Link', label: 'Service Unit Type', fieldname: 'service_unit_type', options: 'Shifaa Service Unit Type'},
		...
		primary_action : function() {
			...
			frappe.call({
				method: 'shifaa.shifaa.doctype.inpatient_record.inpatient_record.schedule_inpatient',
				args: {
					args: args
				},
				callback: function(data) {
	...
	dialog.fields_dict["service_unit_type"].df.onchange = () => {
		if (dialog.get_value("service_unit_type") && dialog.get_value("service_unit_type") != service_unit_type) {
			...
			frappe.db.get_value("Shifaa Service Unit Type", {name: dialog.get_value("service_unit_type")}, ["is_billable", "item"])
			.then(r => {
...
var schedule_discharge = function(frm) {
	var dialog = new frappe.ui.Dialog ({
		...
		primary_action : function() {
			...
			frappe.call ({
				method: 'shifaa.shifaa.doctype.inpatient_record.inpatient_record.schedule_discharge',
				args: {args},
				callback: function(data) {
					if(!data.exc){
						frm.reload_doc();
...
let create_nursing_tasks = function(frm) {
	const d = new frappe.ui.Dialog({
		...
		primary_action: () => {
			...
			frappe.call({
				method: 'shifaa.shifaa.doctype.nursing_task.nursing_task.create_nursing_tasks_from_template',
				args: {
...
let cancel_ip_order = function(frm) {
	...
	function(data) {
		frappe.call({
			method: 'shifaa.shifaa.doctype.inpatient_record.inpatient_record.set_ip_order_cancelled',
			async: false,
			freeze: true,
			args: {
...
let create_service_request = function(frm) {
	frappe.call({
		method: "shifaa.shifaa.doctype.patient_encounter.patient_encounter.create_service_request",
		freeze: true,
		args: {
			encounter: frm.doc.name
		},
...
let create_medication_request = function(frm) {
	frappe.call({
		method: "shifaa.shifaa.doctype.patient_encounter.patient_encounter.create_medication_request",
		freeze: true,
		args: {
			encounter: frm.doc.name
		},
...
var show_clinical_notes = async function(frm) {
	if (frm.doc.docstatus == 0 && frm.doc.patient) {
		const clinical_notes = new shifaa.ClinicalNotes({
			frm: frm,
...
var show_orders = async function(frm) {
	if (frm.doc.docstatus == 0 && frm.doc.patient) {
		const orders = new shifaa.Orders({
			frm: frm,
	...
	var dialog = new frappe.ui.Dialog ({
		...
		primary_action : function() {
			if (dialog.get_value("references").length>0) {
				frappe.call({
					method: "shifaa.shifaa.doctype.patient_encounter.patient_encounter.create_patient_referral",
					freeze: true,
					args: {
						encounter: frm.doc.name,
