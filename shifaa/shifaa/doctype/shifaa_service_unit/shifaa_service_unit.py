...
class ShifaaServiceUnit(NestedSet):
	nsm_parent_field = "parent_shifaa_service_unit"
	...
	def autoname(self):
		if self.company:
			...
			if not self.shifaa_service_unit_name.endswith(suffix):
				self.name = self.shifaa_service_unit_name + suffix
		else:
			self.name = self.shifaa_service_unit_name

	def on_update(self):
		super(ShifaaServiceUnit, self).on_update()
	...
	def set_service_unit_properties(self):
		if cint(self.is_group):
			...
			service_unit_type = frappe.get_doc("Shifaa Service Unit Type", self.service_unit_type)
...
def add_multiple_service_units(parent, data):
	...
	data (dict) - company, shifaa_service_unit_name, count, service_unit_type, warehouse, service_unit_capacity
	"""
	...
	if not data.get("shifaa_service_unit_name") or not company:
		frappe.throw(
			_("Service Unit Name and Company are mandatory to create Shifaa Service Units"),
	...
	service_unit = {
		...
		"parent_shifaa_service_unit": parent if parent != company else None,
	...
	service_unit_name = "{}".format(data.get("shifaa_service_unit_name").strip(" -"))

	last_suffix = frappe.db.sql(
		"""SELECT
		IFNULL(MAX(CAST(SUBSTRING(name FROM %(start)s FOR 4) AS UNSIGNED)), 0)
		FROM `tabShifaa Service Unit`
	...
	for i in range(start_suffix, count + start_suffix):
		...
		service_unit["shifaa_service_unit_name"] = "{}-{}".format(
		...
		try:
			...
			failed_list.append(service_unit["shifaa_service_unit_name"])
...
def on_doctype_update():
	frappe.db.add_unique(
		"Shifaa Service Unit",
		["shifaa_service_unit_name", "company"],
