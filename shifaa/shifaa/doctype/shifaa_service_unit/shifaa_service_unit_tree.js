...
frappe.treeview_settings['Shifaa Service Unit'] = {
	breadcrumbs: 'Shifaa Service Unit',
	title: __('Service Unit Tree'),
	get_tree_root: false,
	get_tree_nodes: 'shifaa.shifaa.utils.get_children',
	add_tree_node: 'shifaa.shifaa.utils.add_node',
	filters: [{
		fieldname: 'company',
		fieldtype: 'Select',
	...
	fields: [
		{
			fieldtype: 'Data', fieldname: 'shifaa_service_unit_name', label: __('New Service Unit Name'),
			reqd: true
		},
		{
		...
		},
		{
			...
			options: 'Shifaa Service Unit Type', description: __('Type of the new Service Unit'),
			...
			onchange: () => {
				if (cur_dialog) {
					if (cur_dialog.fields_dict.service_unit_type.value) {
						frappe.db.get_value('Shifaa Service Unit Type',
	...
	ignore_fields: ['parent_shifaa_service_unit'],
	...
	post_render: function (treeview) {
		frappe.treeview_settings['Shifaa Service Unit'].treeview = {};
		$.extend(frappe.treeview_settings['Shifaa Service Unit'].treeview, treeview);
	},
	toolbar: [
		{
			...
			click: function (node) {
				const dialog = new frappe.ui.Dialog({
					...
					fields: [
						{
							fieldtype: 'Data', fieldname: 'shifaa_service_unit_name', label: __('Service Unit Name'),
					...
					primary_action: () => {
						...
						return frappe.call({
							method: 'shifaa.shifaa.doctype.shifaa_service_unit.shifaa_service_unit.add_multiple_service_units',
							args: {
							...
							callback: function (r) {
								if (!r.exc && r.message) {
									frappe.treeview_settings['Shifaa Service Unit'].treeview.tree.load_children(node, true);
