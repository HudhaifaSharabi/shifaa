...
frappe.ui.form.on('Service Request', {
					...
					.then(r => {
						if (Object.keys(r.message).length == 0) {
							frappe.model.open_mapped_doc({
								method: "shifaa.shifaa.doctype.service_request.service_request.make_appointment",
								frm: frm,
							});
						} else {
							if (r.message && r.message.name) {
	...
	make_clinical_procedure: function(frm) {
		frappe.call({
			method: 'shifaa.shifaa.doctype.service_request.service_request.make_clinical_procedure',
	...
	make_lab_test: function(frm) {
		frappe.call({
			method: 'shifaa.shifaa.doctype.service_request.service_request.make_lab_test',
			args: { service_request: frm.doc },
			freeze: true,
			callback: function(r) {
				var doclist = frappe.model.sync(r.message);
	...
	make_therapy_session: function(frm) {
		frappe.call({
			method: 'shifaa.shifaa.doctype.service_request.service_request.make_therapy_session',
			args: { service_request: frm.doc },
			freeze: true,
			callback: function(r) {
				var doclist = frappe.model.sync(r.message);
	...
	make_observation: function(frm) {
		frappe.call({
			method: 'shifaa.shifaa.doctype.service_request.service_request.make_observation',
			args: { service_request: frm.doc },
			freeze: true,
			callback: function(r) {
				if (r.message) {
