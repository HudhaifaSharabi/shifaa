...
from shifaa.controllers.service_request_controller import ServiceRequestController
...
from shifaa.shifaa.doctype.sample_collection.sample_collection import (
...
class ServiceRequest(ServiceRequestController):
	...
	def set_order_details(self):
		...
		if not self.intent:
			self.intent = frappe.db.get_single_value("Shifaa Settings", "default_intent")

		if not self.priority:
			self.priority = frappe.db.get_single_value("Shifaa Settings", "default_priority")
...
def make_clinical_procedure(service_request):
	...
	if (
		frappe.db.get_single_value("Shifaa Settings", "process_service_request_only_if_paid")
...
def make_appointment(source_name, target_doc=None, ignore_permissions=False):
	...
	def set_missing_values(source, target):
		target.department = frappe.db.get_value(
			"Shifaa Practitioner", source.referred_to_practitioner, "department"
		)
