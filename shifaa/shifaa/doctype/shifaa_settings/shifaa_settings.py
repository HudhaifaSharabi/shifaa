...
def get_sms_text(doc):
	...
	emailed = frappe.db.get_single_value("Shifaa Settings", "sms_emailed")
	sms_text["emailed"] = frappe.render_template(emailed, context)

	printed = frappe.db.get_single_value("Shifaa Settings", "sms_printed")
...
def send_registration_sms(doc):
	if frappe.db.get_single_value("Shifaa Settings", "send_registration_msg"):
		if doc.mobile:
			context = {"doc": doc, "alert": doc, "comments": None}
			if doc.get("_comments"):
				context["comments"] = json.loads(doc.get("_comments"))
			messages = frappe.db.get_single_value("Shifaa Settings", "registration_msg")
...
def get_receivable_account(company):
	receivable_account = get_account(None, "receivable_account", "Shifaa Settings", company)
...
def get_income_account(practitioner, company):
	# check income account in Shifaa Practitioner
	if practitioner:
		income_account = get_account("Shifaa Practitioner", None, practitioner, company)
		if income_account:
			return income_account

	# else check income account in Shifaa Settings
	income_account = get_account(None, "income_account", "Shifaa Settings", company)
	if income_account:
		return income_account
