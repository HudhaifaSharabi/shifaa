...
from shifaa.shifaa.doctype.shifaa_settings.shifaa_settings import (
	get_income_account,
)
from shifaa.shifaa.doctype.lab_test.lab_test import create_multiple
from shifaa.shifaa.doctype.observation.observation import add_observation
from shifaa.shifaa.doctype.observation_template.observation_template import (
	get_observation_template_details,
)
from shifaa.setup import setup_shifaa


@frappe.whitelist()
def get_shifaa_services_to_invoice(patient, customer, company, link_customer=False):
	...
	for appointment in patient_appointments:
		...
		# Consultation Appointments, should check fee validity
			...
			if frappe.db.get_single_value(
				"Shifaa Settings", "enable_free_follow_ups"
			...
			if not encounter.appointment:
				...
				if encounter.practitioner:
					if encounter.inpatient_record and frappe.db.get_single_value(
						"Shifaa Settings", "do_not_bill_inpatient_encounters"
					):
						continue
	...
	for procedure in procedures:
		...
		if (
			...
			service_item = frappe.db.get_single_value(
				"Shifaa Settings", "clinical_procedure_consumable_item"
			)
			if not service_item:
				msg = _("Please Configure Clinical Procedure Consumable Item in {0}").format(
					get_link_to_form("Shifaa Settings", "Shifaa Settings")
				)
...
def get_inpatient_services_to_invoice(patient, company):
	...
	for inpatient_occupancy in inpatient_services:
		service_unit_type = frappe.db.get_value(
			"Shifaa Service Unit", inpatient_occupancy.service_unit, "service_unit_type"
		)
		service_unit_type = frappe.get_cached_doc("Shifaa Service Unit Type", service_unit_type)
...
def get_appointment_billing_item_and_rate(doc):
	...
	if not service_item:
		service_item = get_shifaa_service_item(is_inpatient)
...
def get_appointment_type_billing_details(appointment_type, dep_su, is_inpatient):
	from shifaa.shifaa.doctype.appointment_type.appointment_type import get_billing_details
...
def throw_config_service_item(is_inpatient):
	...
	msg = _(
		...
		+ """<b><a href='/app/Form/Shifaa Settings'>Shifaa Settings</a></b>"""
	)
	frappe.throw(msg, title=_("Missing Configuration"))
...
def throw_config_practitioner_charge(is_inpatient, practitioner):
	...
	msg = _(
		("Please Configure {0} for Shifaa Practitioner").format(charge_name)
		+ """ <b><a href='/app/Form/Shifaa Practitioner/{0}'>{0}</a></b>""".format(practitioner)
	)
	frappe.throw(msg, title=_("Missing Configuration"))
...
def get_practitioner_billing_details(practitioner, is_inpatient):
	...
	if practitioner:
		service_item, practitioner_charge = frappe.db.get_value(
			"Shifaa Practitioner", practitioner, fields
		)

	return service_item, practitioner_charge


def get_shifaa_service_item(is_inpatient):
	service_item = None

	if is_inpatient:
		service_item = frappe.db.get_single_value("Shifaa Settings", "inpatient_visit_charge_item")
	else:
		service_item = frappe.db.get_single_value("Shifaa Settings", "op_consulting_charge_item")
...
def manage_invoice_submit_cancel(doc, method):
	...
	if doc.items:
		...
		if method == "on_submit" and frappe.db.get_single_value(
			"Shifaa Settings", "create_observation_on_si_submit"
		):
			create_sample_collection_and_observation(doc)

	if method == "on_submit":
		if frappe.db.get_single_value("Shifaa Settings", "create_lab_test_on_si_submit"):
			create_multiple("Sales Invoice", doc.name)

		if (
			not frappe.db.get_single_value("Shifaa Settings", "show_payment_popup")
			and frappe.db.get_single_value("Shifaa Settings", "enable_free_follow_ups")
			and doc.items
		):
			for item in doc.items:
	...
	if item.reference_dt == "Patient Appointment":
		...
		if item.reference_dt == "Service Request":
			template_map = {
				...
				# 'Shifaa Service Unit': 'Inpatient Occupancy'
			}
...
def validate_invoiced_on_submit(item):
	if (
		...
		and frappe.db.get_single_value("Shifaa Settings", "clinical_procedure_consumable_item")
		== item.item_code
	):
...
def get_children(doctype, parent=None, company=None, is_root=False):
	...
	if is_root:
		fields += ["service_unit_type"] if doctype == "Shifaa Service Unit" else []
		filters.append(["company", "=", company])
	else:
		fields += (
			["service_unit_type", "allow_appointments", "inpatient_occupancy", "occupancy_status"]
			if doctype == "Shifaa Service Unit"
			else []
		)
	...
	for each in service_units:
		if each["expandable"] != 1 or each["value"].startswith("All Shifaa Service Units"):
			continue

		available_count = frappe.db.count(
			"Shifaa Service Unit",
			filters={"parent_shifaa_service_unit": each["value"], "inpatient_occupancy": 1},
		)
		...
		if available_count > 0:
			occupied_count = frappe.db.count(
				"Shifaa Service Unit",
				filters={
					"parent_shifaa_service_unit": each["value"],
...
def update_address_links(address, method):
	...
	"""
	if "Shifaa" not in frappe.get_active_domains():
		return
...
def update_patient_email_and_phone_numbers(contact, method):
	...
	"""
	if "Shifaa" not in frappe.get_active_domains() or contact.flags.skip_patient_update:
		return
...
def create_shifaa_service_unit_tree_root(doc, method=None):
	record = [
		{
			"doctype": "Shifaa Service Unit",
			"shifaa_service_unit_name": "All Shifaa Service Units",
			"is_group": 1,
			"company": doc.name,
		}
	]
...
def validate_nursing_tasks(document):
	if not frappe.db.get_single_value("Shifaa Settings", "validate_nursing_checklists"):
		return True
...
def company_on_trash(doc, method):
	for su in frappe.get_all("Shifaa Service Unit", {"company": doc.name}):
		service_unit_doc = frappe.get_doc("Shifaa Service Unit", su.get("name"))
...
def add_node():
	...
	if cint(args.is_root):
		args.parent_shifaa_service_unit = None
