...
frappe.ui.form.on('Sales Invoice', {
	refresh(frm) {
		if (frm.doc.docstatus === 0 && !frm.doc.is_return) {
			frm.add_custom_button(__('Shifaa Services'), function() {
				if (!frm.doc.patient) {
				...
				.then(r => {
					...
					if (r.message.customer){
						get_shifaa_services_to_invoice(frm, link_customer);
					} else {
						frappe.confirm(msg,
							() => {
								link_customer = true;
...
var get_shifaa_services_to_invoice = function(frm, link_customer) {
	var me = this;
	let selected_patient = '';
	var dialog = new frappe.ui.Dialog({
		title: __("Get Items from Shifaa Services"),
		fields:[
			{
				fieldtype: 'Link',
				options: 'Patient',
	...
	dialog.fields_dict["patient"].df.onchange = () => {
		...
		if(patient && patient!=selected_patient){
			...
			var method = "shifaa.shifaa.utils.get_shifaa_services_to_invoice";
			var args = {patient: patient, customer: frm.doc.customer, company: frm.doc.company, link_customer: link_customer};
			var columns = (["service", "reference_name", "reference_type"]);
			get_shifaa_items(frm, true, $results, $placeholder, method, args, columns);
		}
		else if(!patient){
			selected_patient = '';
			$results.empty();
	...
	$placeholder = $(`<div class="multiselect-empty-state">
				<span class="text-center" style="margin-top: -40px;">
					...
					<p class="text-extra-muted">No billable Shifaa Services found</p>
				</span>
			</div>`);
...
var get_shifaa_items = function(frm, invoice_shifaa_services, $results, $placeholder, method, args, columns) {
	var me = this;
	$results.empty();
	frappe.call({
		method: method,
		...
		callback: function(data) {
			if(data.message){
				$results.append(make_list_row(columns, invoice_shifaa_services));
				for(let i=0; i<data.message.length; i++){
					$results.append(make_list_row(columns, invoice_shifaa_services, data.message[i]));
				}
			}else {
				$results.append($placeholder);
			}
...
var make_list_row= function(columns, invoice_shifaa_services, result={}) {
	var me = this;
	...
	$row = list_row_data_items(head, $row, result, invoice_shifaa_services);
	return $row;
};

var set_primary_action= function(frm, dialog, $results, invoice_shifaa_services) {
	var me = this;
	dialog.set_primary_action(__('Add'), function() {
//		frm.clear_table('items');
		let checked_values = get_checked_values($results);
		if(checked_values.length > 0){
			if(invoice_shifaa_services) {
				frm.set_value("patient", dialog.fields_dict.patient.input.value);
			}
			add_to_item_line(frm, checked_values, invoice_shifaa_services);
			dialog.hide();
		}
		else{
			if(invoice_shifaa_services){
				frappe.msgprint(__("Please select Shifaa Service"));
			}
			else{
...
var get_drugs_to_invoice = function(frm, link_customer) {
	...
	dialog.fields_dict["encounter"].df.onchange = () => {
		...
		if(encounter && encounter!=selected_encounter){
			...
			var method = "shifaa.shifaa.utils.get_drugs_to_invoice";
			...
			get_shifaa_items(frm, false, $results, $placeholder, method, args, columns);
		}
		else if(!encounter){
			selected_encounter = '';
			$results.empty();
...
var list_row_data_items = function(head, $row, result, invoice_shifaa_services) {
	if(invoice_shifaa_services){
...
var add_to_item_line = function(frm, checked_values, invoice_shifaa_services){
	if(invoice_shifaa_services){
		frappe.call({
			doc: frm.doc,
			method: "set_shifaa_services",
			args:{
